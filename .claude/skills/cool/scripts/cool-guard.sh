#!/bin/bash
# Cool Phase Guard — validates exit conditions before phase transitions
# Usage: cool-guard.sh <change-name> <current-phase> [--apply]
...
if [ "${COOL_GUARD_SOURCE_ONLY:-0}" = "1" ]; then
  CHANGE="${CHANGE:-}"
  PHASE="${PHASE:-}"
  APPLY="${APPLY:-0}"
...
plan_tasks_all_done() {
  ...
  if [ ! -f "$plan" ]; then
    ...
    echo "Next: restore the Superpowers plan file or update .cool.yaml plan before leaving build." >&2
    return 1
  fi
...
yaml_field_value() {
  local field="$1"
  local yaml="$CHANGE_DIR/.cool.yaml"
  if [ -f "$yaml" ]; then
    local value
...
# Read a config-overridable field following the canonical priority:
#   1. per-change .cool.yaml  2. COOL_<FIELD> env  3. .cool/config.yaml
# Echoes the value and returns 0 if found, returns 1 if absent (caller falls back).
# Used for build_command / verify_command. Aligns with the documented five-layer
# priority (CLI flag writes per-change, so per-change is highest).
project_config_value() {
  local field="$1"
  local env_var=""
  case "$field" in
    build_command)  env_var="COOL_BUILD_COMMAND" ;;
    verify_command) env_var="COOL_VERIFY_COMMAND" ;;
  esac

  local value

  # 1. per-change .cool.yaml (highest; includes CLI-flag-written values)
  ...
  # 2. environment variable (session-level override)
  if [ -n "$env_var" ] && [ -n "${!env_var:-}" ]; then
    echo "${!env_var}"
    return 0
  fi

  # 3. project .cool/config.yaml (team/project default)
  if [ -f ".cool/config.yaml" ]; then
    value=$(grep "^${field}:" ".cool/config.yaml" 2>/dev/null | sed "s/^${field}: *//" || true)
    value=$(strip_inline_comment "$value")
...
# Returns the path to the current project's CONTEXT.md.
# L1/L2判定: if cwd root contains assets/skills/cool/SKILL.md → L1 (cool-flow repo itself);
# otherwise L2 (user project root).
current_project_context_md() {
  if [ -f "assets/skills/cool/SKILL.md" ]; then
    echo "assets/skills/cool/CONTEXT.md"
  else
    echo "CONTEXT.md"
  fi
}

# warn-level existence check for CONTEXT.md (does not set BLOCK=1).
# Conditionally triggered: L1 always warns when missing; L2 warns only when
# the project has opted into domain modeling (adr/ exists), otherwise silent
# (backward compatible — L2 lazily creates CONTEXT.md on first term involvement).
context_md_exists_warn() {
  local ctx
  ctx="$(current_project_context_md 2>/dev/null || true)"
  if [ -s "$ctx" ]; then
    return 0
  fi
  if [ "$ctx" = "assets/skills/cool/CONTEXT.md" ]; then
    warn "  [WARN] L1 CONTEXT.md missing or empty ($ctx). Domain language single source not established."
    return 0
  fi
  # L2: warn only if project has opted into domain modeling (adr/ exists)
  if [ -d "adr" ]; then
    warn "  [WARN] L2 CONTEXT.md missing or empty ($ctx) while adr/ exists. Run cool-design domain-modeling discipline or create CONTEXT.md."
  fi
  return 0
}
...
preflight() {
  ...
  if [ ! -f "$CHANGE_DIR/.cool.yaml" ]; then
    red "FATAL: .cool.yaml not found in $CHANGE_DIR"
    exit 1
  fi

  # Schema validation
  local validate_script
  validate_script="$SCRIPT_DIR/cool-yaml-validate.sh"
  if [ -f "$validate_script" ]; then
    if ! "$COOL_BASH" "$validate_script" "$CHANGE" 2>/dev/null; then
      "$COOL_BASH" "$validate_script" "$CHANGE" || true
      red "FATAL: .cool.yaml schema validation failed"
      exit 1
    fi
  fi
}
...
build_passes() {
  ...
  red "ERROR: no build_command configured and no supported build manifest detected" >&2
  red "  looked in: per-change .cool.yaml, COOL_BUILD_COMMAND, .cool/config.yaml" >&2
  red "  fix: set 'build_command:' in .cool/config.yaml, or export COOL_BUILD_COMMAND=..." >&2
  red "  (or set COOL_SKIP_BUILD=1 to skip build checks)" >&2
  return 1
}
...
isolation_selected() {
  ...
  case "$isolation" in
    ...
    *)
      ...
      echo "  \"\$COOL_BASH\" \"\$COOL_STATE\" set $CHANGE isolation <branch|worktree>" >&2
      return 1
      ;;
  esac
}
...
build_mode_selected() {
  ...
  case "$build_mode" in
    ...
    *)
      ...
      echo "  \"\$COOL_BASH\" \"\$COOL_STATE\" set $CHANGE build_mode <subagent-driven-development|executing-plans>" >&2
      return 1
      ;;
  esac
}
...
subagent_dispatch_confirmed() {
  ...
  echo "  \"\$COOL_BASH\" \"\$COOL_STATE\" set $CHANGE subagent_dispatch confirmed" >&2
  echo "Or ask the user to switch to executing-plans and run:" >&2
  echo "  \"\$COOL_BASH\" \"\$COOL_STATE\" set $CHANGE build_mode executing-plans" >&2
  return 1
}
...
tdd_mode_selected() {
  ...
  case "$tdd_mode" in
    ...
    *)
      ...
      echo "  \"\$COOL_BASH\" \"\$COOL_STATE\" set $CHANGE tdd_mode <tdd|direct>" >&2
      return 1
      ;;
  esac
}
...
review_result_is_pass() {
  local result
  result=$(yaml_field_value "review_result" 2>/dev/null || true)
  [ "$result" = "pass" ] && return 0
  echo "review_result must be 'pass' before leaving review, got '${result:-null}'" >&2
  echo "Next: complete /cool-review and ensure review passes before running guard" >&2
  return 1
}

review_report_exists() {
  local report
  report=$(yaml_field_value "review_report" 2>/dev/null || true)
  if [ -n "$report" ] && [ "$report" != "null" ] && [ -f "$report" ]; then
    return 0
  fi
  echo "review_report must point to an existing report file, got '${report:-null}'" >&2
  echo "Next: complete /cool-review to generate the review report" >&2
  return 1
}
...
design_handoff_context_valid() {
  ...
  if [ -z "$context" ] || [ "$context" = "null" ]; then
    echo "handoff_context is missing from .cool.yaml" >&2
    echo "Next: run \"\$COOL_BASH\" \"\$COOL_HANDOFF\" $CHANGE design --write before invoking Superpowers." >&2
    return 1
  fi
  if [ ! -s "$context" ]; then
    echo "handoff_context does not point to a non-empty file: $context" >&2
    echo "Next: regenerate the design handoff with cool-handoff.sh." >&2
    return 1
  fi
  ...
  if [ "$actual_hash" != "$recorded_hash" ]; then
    ...
    echo "Next: rerun cool-handoff.sh so Superpowers receives the current OpenSpec context." >&2
    return 1
  fi
...
design_handoff_markdown_traceable() {
  ...
  grep -q '^Generated-by: cool-handoff.sh$' "$markdown" || {
...
design_doc_links_current_change() {
  ...
  design_doc_frontmatter_has "$design_doc" "cool_change" "$CHANGE"
}
...
guard_open() {
  ...
  check "open_branch recorded (not on a protected branch)" open_branch_recorded
  check "current branch matches open_branch" current_branch_matches_open_branch
}

open_branch_recorded() {
  local ob
  ob=$(yaml_field_value "open_branch" 2>/dev/null || true)
  if [ -z "$ob" ] || [ "$ob" = "null" ]; then
    echo "open_branch must be recorded before leaving open phase (protected branches must not hold change artifacts)." >&2
    echo "Next: \"\$COOL_BASH\" \"\$COOL_STATE\" open-branch $CHANGE [--branch <name>]" >&2
    return 1
  fi
  return 0
}

current_branch_matches_open_branch() {
  local ob cur
  ob=$(yaml_field_value "open_branch" 2>/dev/null || true)
  cur=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "")
  if [ -z "$cur" ] || [ "$cur" = "HEAD" ]; then
    echo "detached HEAD or non-git; expected open_branch='$ob'" >&2
    return 1
  fi
  if [ "$cur" != "$ob" ]; then
    echo "current branch '$cur' != open_branch '$ob'. Run: git checkout '$ob'" >&2
    return 1
  fi
  return 0
}
...
guard_design() {
  ...
  if [ -n "$design_doc" ] && [ "$design_doc" != "null" ]; then
    ...
    warn "  [WARN] No design_doc recorded in .cool.yaml (optional for hotfix/tweak)"
  fi

  # Domain language layer: CONTEXT.md existence warn (L1/L2 aware, does not block).
  context_md_exists_warn
}
...
design_doc_recorded() {
  ...
  echo "Next: create the Design Doc and run: \"\$COOL_BASH\" \"\$COOL_STATE\" set $CHANGE design_doc <path>" >&2
  return 1
}
...
guard_build() {
  ...
  if [ "$review_mode" = "skip" ]; then
    echo "=== Guard: build → verify (review skipped: tweak) ===" >&2
  else
    echo "=== Guard: build → review ===" >&2
  fi
...
guard_review() {
  echo "=== Guard: review → verify ===" >&2

  check "review_result is pass" review_result_is_pass
  check "review_report exists" review_report_exists
}
...
next_phase_of() {
  case "$1" in
    ...
    verify) printf 'archive' ;;
    archive) printf '' ;;   # 终态
    *)      printf '' ;;
  esac
}

# run_phase_extensions <current-phase>
# 在 --apply transition 前触发 post-<current> + pre-<next>。
# runner 不存在/不可执行 → warn 降级 no-op（不阻断，向后兼容旧版）。
# 返回 0=pass/warn；1=block（调用方设 BLOCK=1，不 transition）
run_phase_extensions() {
  local current="$1"
  local ext="${COOL_EXTENSION_SH:-$SCRIPT_DIR/cool-extension.sh}"
  if [ -z "$ext" ] || [ ! -x "$ext" ]; then
    # 降级 no-op：向后兼容旧版（无 cool-extension.sh）
    return 0
  fi
  local next
  next=$(next_phase_of "$current")

  # pre-validation：transition 前先校验扩展配置，配置畸形 fail-fast 而非运行时才暴露（W5）。
  local vout
  vout=$("$COOL_BASH" "$ext" validate "$CHANGE" 2>&1) || {
    red "  [EXT] extension config invalid — fix before transition:"
    printf '%s\n' "$vout" >&2
    return 1
  }

  if ! "$COOL_BASH" "$ext" run "$CHANGE" "$current" post; then
    red "  [EXT] post-$current hook BLOCKED transition"
    return 1
  fi
  if [ -n "$next" ]; then
    if ! "$COOL_BASH" "$ext" run "$CHANGE" "$next" pre; then
      red "  [EXT] pre-$next hook BLOCKED transition"
      return 1
    fi
  fi
  return 0
...
apply_state_update() {
  local state_sh="$SCRIPT_DIR/cool-state.sh"
  local p="$1"

  if [ -f "$state_sh" ]; then
    case "$p" in
      open)   "$COOL_BASH" "$state_sh" transition "$CHANGE" open-complete ;;
      design) "$COOL_BASH" "$state_sh" transition "$CHANGE" design-complete ;;
      build)  "$COOL_BASH" "$state_sh" transition "$CHANGE" build-complete ;;
      review) "$COOL_BASH" "$state_sh" transition "$CHANGE" review-pass ;;
      verify) "$COOL_BASH" "$state_sh" transition "$CHANGE" verify-pass ;;
    esac
  else
    red "FATAL: cool-state.sh not found; cannot apply state transition"
    exit 1
  fi
}
...
if [ "${COOL_GUARD_SOURCE_ONLY:-0}" = "1" ]; then
  ...
  red "ERROR: COOL_GUARD_SOURCE_ONLY=1 is only for sourcing, not direct execution" >&2
...
case "$PHASE" in
  ...
  review)   preflight ; guard_review ;;
  verify)   preflight ; guard_verify ;;
  archive)  preflight ; guard_archive ;;
  *)
    red "Unknown phase: $PHASE"
    echo "Valid phases: open, design, build, review, verify, archive" >&2
    exit 1
    ;;
esac
...
if [ "$BLOCK" -eq 1 ]; then
  ...
  if [ "$APPLY" -eq 1 ]; then
    if ! run_phase_extensions "$PHASE"; then
      echo "" >&2
      red "BLOCKED — extension hook blocked transition; fix hook then re-run guard --apply"
      exit 1
    fi
    ...
    case "$PHASE" in
      open)
        ...
        green "  [APPLY] .cool.yaml updated: phase=$new_phase"
        ;;
      design) green "  [APPLY] .cool.yaml updated: phase=build" ;;
      build)  green "  [APPLY] .cool.yaml updated: phase=verify, verify_result=pending" ;;
      review) green "  [APPLY] .cool.yaml updated: phase=verify, review_result=pass" ;;
      verify) green "  [APPLY] .cool.yaml updated: phase=archive, verify_result=pass" ;;
    esac
  fi
  exit 0
fi
