  ...
  <view class="container">
    <view v-if="service" class="service-card">
      ...
      <view class="service-info">
        <image class="service-image" :src="service.image_url || '/images/default-service.png'" />
...
const submitOrder = async () => {
  ...
  try {
    ...
    Taro.hideLoading()
    Taro.showToast({ title: '订单创建成功！', icon: 'success', duration: 2000 })
    setTimeout(() => {
...
Taro.useLoad((options) => {
  if (options && options.service) {
    try {
      ...
      Taro.showToast({ title: '服务信息错误', icon: 'none', duration: 2000 })
      setTimeout(() => Taro.navigateBack(), 2000)
    }
  } else {
    Taro.showToast({ title: '未获取到服务信息', icon: 'none', duration: 2000 })
    setTimeout(() => Taro.navigateBack(), 2000)
  }
})
</script>
